...
func TestKeyring_EnvironmentVariableTakesPrecedence(t *testing.T) {
	...
	// Set env var - keyring should still take precedence
	...
	if token != keyringToken {
		t.Errorf("keyring should take precedence: expected %q, got %q", keyringToken, token)
	}

	// Clean up
	_ = DeleteToken()

	// Now with no keyring token, should get env var
	token, err = GetToken()
	if err != nil {
